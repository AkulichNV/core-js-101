...
function isCreditCardNumber(ccn) {
  let cardNumers = ccn.toString().split('');
  cardNumers = cardNumers.map(Number);
  cardNumers.reverse();
  let sum = 0;
  for (let i = 0; i < cardNumers.length; i += 1) {
...
function evaluateTicTacToePosition(position) {
  ...
  for (let k = 0; k < position.length; k += 1) {
    if (position[k].length < 3) {
      position[k].push(undefined);
    }
  }
  let pos0 = position.reduce((acc, val) => acc.concat(val), []);
  let posX = position.reduce((acc, val) => acc.concat(val), []);

  for (let i = 0; i < pos0.length; i += 1) {
    if (posX[i] === 'X') {
      posX[i] = i;
      pos0[i] = undefined;
    } else if (pos0[i] === '0') {
  ...
  pos0 = pos0.filter((element) => element !== undefined);
  posX = posX.filter((element) => element !== undefined);

  for (let j = 0; j < arrWinIndex.length; j += 1) {
    const contains0 = arrWinIndex[j].every((element) => pos0.includes(element));
    const containsX = arrWinIndex[j].every((element) => posX.includes(element));
    if (contains0) {
      return '0';
    }
    if (containsX) {
